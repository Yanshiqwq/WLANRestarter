...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGyX8VAjFBdRQguUM1eeA6YX/Ofr092rg2MvUfEreZ3P36bDNrFduAvhbZNN
::YB416Ek+ZW8=
::
::
...
for /f "tokens=3" %i in ('route print -4^|find "0.0.0.0"') do @(for /f "eol=ÔÚ" %n in ('echo %i') do @(set routeip=%n))
echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Router IP is %ROUTEIP%.
echo.
...
if "%1" == "-log" (
	...
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Router IP is %ROUTEIP%. >> %LOGFILE%
	echo. >> %LOGFILE%
	goto :looplog
)
:loop
...
if "%ERRORLEVEL%" == "1" (
	...
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Ping to subrouter %ROUTEIP% ...
	ping %ROUTEIP% -n 1 > nul
	ping %ROUTEIP% -n 1 > nul
	if "%ERRORLEVEL%"=="1" (
		echo [%time:~0,2%:%time:~3,2%:%time:~6,2% FATAL] FAILED to ping subrouter %ROUTEIP%.
...
if "%ERRORLEVEL%"=="1" (
	...
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% ERROR] FAILED to ping router 192.168.1.1.
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Ping to subrouter %ROUTEIP%... >> %LOGFILE%
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Ping to subrouter %ROUTEIP%...
	ping %ROUTEIP% -n 1 > nul
    	ping %ROUTEIP% -n 1 > nul
	if "%ERRORLEVEL%"=="1" (
		echo [%time:~0,2%:%time:~3,2%:%time:~6,2% FATAL] FAILED to ping subrouter %ROUTEIP%. >> %LOGFILE%
		echo [%time:~0,2%:%time:~3,2%:%time:~6,2% FATAL] FAILED to ping subrouter %ROUTEIP%.
	)
