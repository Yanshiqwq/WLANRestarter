...
if "%1" == "-log" (
	echo. >> %LOGFILE%
	echo. >> %LOGFILE%
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] AutoRestartWLAN.bat running at [%date:~0,4%.%date:~5,2%.%date:~8,2% %time:~0,2%:%time:~3,2%:%time:~6,2%]. >> %LOGFILE%
	echo. >> %LOGFILE%
	goto :looplog
)
:loop
set /a COUNT=COUNT+1
echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Ping to router 192.168.199.1 ^(Count=%COUNT%^)...
ping 192.168.199.1 -n 1>nul
if "%ERRORLEVEL%" == "1" (
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% ERROR] FAILED to ping router 192.168.199.1
...
:looplog
echo [%time:~0,2%:%time:~3,2%:%time:~6,2% INFO] Ping to router 192.168.199.1 ^(Count=%COUNT%^)... >> %LOGFILE%
ping 192.168.199.1 -n 1>nul
if "%ERRORLEVEL%"=="1" (
	echo [%time:~0,2%:%time:~3,2%:%time:~6,2% ERROR] FAILED to ping router 192.168.199.1 >> %LOGFILE%
...
:loopping
ping -n 1 192.168.199.1 -w 10>nul
